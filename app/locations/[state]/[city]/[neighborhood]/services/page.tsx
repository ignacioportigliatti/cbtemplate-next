  ...
  try {
    ...
    if (!seoLocationData) {
      return {
        ...
        description: "The requested neighborhood services could not be found",
    ...
    const description = `Professional services available in ${seoLocationData.address.neighborhood}, ${seoLocationData.address.city}, ${seoLocationData.address.state}. Browse our complete service offerings.`;
    const logoUrl = themeOptions.general.site_logo?.url;

    return {
      title: `Services in ${seoLocationData.address.neighborhood}, ${seoLocationData.address.city}, ${seoLocationData.address.state} | ${title}`,
      description,
      openGraph: {
        title: `Services in ${seoLocationData.address.neighborhood}, ${seoLocationData.address.city}, ${seoLocationData.address.state} | ${title}`,
        description,
        type: "website",
        url: `${siteConfig.site_domain}/locations/${state}/${city}/${neighborhood}/services`,
    ...
    console.error('Error generating metadata for neighborhood services page:', error);
    return {
...
export default async function NeighborhoodServicesPage({ params }: { 
  params: Promise<{ state: string; city: string; neighborhood: string }> 
}) {
  const { state, city, neighborhood } = await params;
  ...
  try {
    ...
    return (
      <>
        {/* Structured Data for Neighborhood Services Page */}
        {business && (
          <LocalBusinessSchema 
            business={business}
            services={services}
        ...
        {/* Template Component */}
        <template.NeighborhoodServicesPage 
    ...
    console.error('Error in NeighborhoodServicesPage:', error);
    // Return template without structured data if there's an error
    return <template.NeighborhoodServicesPage seoLocationData={null} mainLocationData={null} servicesContent={null} contactContent={null} themeOptions={null} />;
  }
}
