...
import { PostCard } from "@/templates/barbershop/components/blog/post-card";
import { FilterPosts } from "@/templates/barbershop/components/blog/filter";
import { SearchInput } from "@/templates/barbershop/components/blog/search-input";
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    const description = "Browse all our blog posts";
    ...
    return {
      ...
      openGraph: {
        ...
        images: [
          {
            url: `${siteConfig.site_domain}/api/og?title=${encodeURIComponent(`Blog | ${title}`)}&description=${encodeURIComponent(description)}${logoUrl ? `&logo=${encodeURIComponent(logoUrl)}` : ''}`,
            width: 1200,
            height: 630,
            alt: title,
          },
      ...
      twitter: {
        ...
        title: `Blog | ${title}`,
        description: description,
        images: [`${siteConfig.site_domain}/api/og?title=${encodeURIComponent(`Blog | ${title}`)}&description=${encodeURIComponent(description)}${logoUrl ? `&logo=${encodeURIComponent(logoUrl)}` : ''}`],
      },
    };
  } catch (error) {
